	...
	TextGeometry: function ( mesh ) {
		...
		function generateGeometry() {
			...
			loader.load( '../../examples/fonts/' + data.font + '_' + data.weight + '.typeface.json', function ( font ) {
				...
				var textGeometry = new THREE.TextGeometry( data.text, {
					font: font,
					size: data.size,
					height: data.height,
				...
				updateGroupGeometry( mesh, textGeometry );
				var Txt3D = new THREE.Mesh( textGeometry, meshMaterial );
				Txt3D.name = "txt";

				for (var i = group.children.length - 1; i >= 0; i--) 
				{
					if (group.children[i].name == "txt")
    					group.remove(group.children[i]);
				}

				Txt3D.castShadow = true;
				Txt3D.receiveShadow = false;
				// textShadow = new THREE.ShadowMesh( Txt3D );
				// group.add( textShadow );
				group.add(Txt3D)



				var modifier = new THREE.BendModifier();
				modifier.modify( textGeometry, Txt3D.geometry, data.angle, data.center );
			} );
		...
		// folder.add( data, 'height', -20, 20 ).onChange( generateGeometry );
		...
		// folder.add( data, 'angle', -10,10).step(0.001).onChange(generateGeometry);
		// folder.add( data, 'center', -2,2).onChange(generateGeometry);
...
function chooseFromHash( mesh, geometry, meshMaterial, group, scene) {
